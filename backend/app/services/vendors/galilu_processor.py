...
class GaliluProcessor:
    """Process Galilu Excel files with PLN to EUR conversion"""

    TARGET_SHEET = "Sheet1"
    PLN_TO_EUR_RATE = 0.23  # Approximate rate, should be configurable
    ...
    def __init__(self, exchange_rate: Optional[float] = None):
        ...
        self.exchange_rate = exchange_rate or self.PLN_TO_EUR_RATE
    ...
    def process(
        ...
        """
        Process Galilu file with PLN to EUR conversion
        ...
        try:
            ...
            raise Exception(f"Failed to process Galilu file: {str(e)}")
    ...
    def _transform_row(
        ...
        """
        Transform raw row with PLN to EUR conversion

        Note: Galilu may not have EAN for all products
        """
        transformed = {
            "user_id": user_id,
            "batch_id": batch_id,
        ...
        for source_col, target_col in self.COLUMN_MAPPING.items():
            ...
            # Handle EAN - can be NULL for Galilu
            if target_col == "product_ean":
            ...
            elif target_col == "sales_pln":
                if not value:
                    raise ValueError("Missing required field: Amount")
                pln_amount = self._to_float(value, "Amount")
                # Convert PLN to EUR
                transformed["sales_eur"] = round(pln_amount * self.exchange_rate, 2)
            elif target_col == "month":
                if not value:
                    raise ValueError("Missing required field: Month")
                ...
                transformed[target_col] = str(value) if value else "Galilu"
            else:
    ...
    def _validate_ean(self, value: Any) -> Optional[str]:
        """Validate EAN code - can be None for Galilu"""
        if not value:
            return None
        ...
        if len(ean_str) != 13 or not ean_str.isdigit():
            return None  # Return None instead of raising for Galilu
